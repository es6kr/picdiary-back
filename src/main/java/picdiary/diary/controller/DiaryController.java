...
public class DiaryController {
     ...
     * 다이어리 수정
     */
    @PatchMapping(value = "/{date}", consumes = APPLICATION_JSON_VALUE)
    public ApplicationResponse<Long> updateDiary(@Parameter(hidden = true) UserEntity user, @PathVariable("date") String date, @RequestBody DiaryUpdateRequest request) {
        var diaryId = diaryService.getDiaryByMonth(user.getId(), date).stream().findFirst().orElseThrow().getId();
        createDiaryImageFile(user, request, date);
